...
// دالة مساعدة شاملة للتعامل مع النصوص المترجمة
function getTxt(obj: Localized | string | undefined | null, locale: string): string {
  if (!obj) return "";
...
export async function generateStaticParams() {
  try {
    ...
    return locales.flatMap((locale) => slugs.map((p) => ({ locale, slug: p.slug })));
  } catch {
    return [];
  }
}
...
export async function generateMetadata({ params }: ProjectPageProps): Promise<Metadata> {
  ...
  try {
    ...
    const tr = (project.translations as ProjectTranslation[]).find((x) => x.locale === locale);
...
export default async function ProjectDetailPage({ params }: ProjectPageProps) {
  ...
  let project: ProjectDetailPayload | null = null;
  try {
  ...
  // حالة عدم وجود مشروع في قاعدة البيانات (Fallback logic)
  if (!project) {
    ...
    return (
      <article className="container section">
        ...
        <section className="section" style={{ paddingTop: 0 }}>
          <h2 className="h2">{t(locale, "project.gallery")}</h2>
          <div className="card-grid">
            {fallback.gallery.map((src, idx) => (
              <div key={src} className="card project-gallery-item">
                <Image src={src} alt={getTxt(fallback.title, locale)} width={1200} height={700} />
        ...
        <section className="section">
          <div className="card-grid">
            <article className="card"><h2>Overview</h2><p>{getTxt(fallback.overview, locale)}</p></article>
            <article className="card"><h2>Problem</h2><p>{getTxt(fallback.problem, locale)}</p></article>
            <article className="card"><h2>Solution</h2><p>{getTxt(fallback.solution, locale)}</p></article>
            <article className="card"><h2>Technologies</h2><p>{fallback.technologies.join(" • ")}</p></article>
            <article className="card"><h2>Scope</h2><p>{fallback.scope.join(" • ")}</p></article>
            <article className="card"><h2>Outcome</h2><p>{getTxt(fallback.outcome, locale)}</p></article>
          </div>
          <div className="hero-actions" style={{ marginTop: "1rem" }}>
            <Link className="btn btn-primary" href={`/${locale}/contact`}>{isIpbx ? "Request IPBX Consultation" : "Start a similar project"}</Link>
            <Link className="btn btn-ghost" href={localePath(locale, "/projects")}>Back to projects</Link>
          </div>
        </section>
  ...
  // في حالة وجود المشروع (DB Path) - يتم استخدام getTxt أيضاً
  return (
    <article className="container section">
