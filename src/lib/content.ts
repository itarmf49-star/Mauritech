...
export const services: Service[] = [
  { id: "residential-internet", title: { fr: "Internet Résidentiel", ar: "إنترنت منزلي" }, icon: "Home", description: { fr: "Solutions fiables.", ar: "حلول موثوقة." }, image: "/images/hero-en.svg", href: "/services/res", features: ["Planification", "Installation"] },
  { id: "home-wifi", title: { fr: "Couverture Wi-Fi", ar: "تغطية الواي فاي" }, icon: "Wifi", description: { fr: "Systèmes Mesh.", ar: "أنظمة المش." }, image: "/images/hero-en.svg", href: "/services/wifi", features: ["Analyse", "Déploiement"] },
  { id: "business-networks", title: { fr: "Réseaux Entreprise", ar: "شبكات الأعمال" }, icon: "Building2", description: { fr: "Sécurisé.", ar: "آمنة." }, image: "/images/hero-en.svg", href: "/services/bus", features: ["LAN/WAN", "VLAN"] },
  { id: "infrastructure", title: { fr: "Infrastructure", ar: "البنية التحتية" }, icon: "Network", description: { fr: "Câblage structuré.", ar: "كابلات مهيكلة." }, image: "/images/hero-en.svg", href: "/services/infra", features: ["Câblage", "Armoires"] }
];

const projectEntries: Omit<Project, 'odooId' | 'status' | 'synced'>[] = [
  {
...
export function getProjectsByCategory(cat: string) { return projects.filter((p) => p.category.fr.includes(cat) || p.category.ar.includes(cat)); }
export function getFeaturedProjects() { return projects.filter((p) => p.featured); }
export const networkingProjectSlugs = new Set(["enterprise-network-deployment", "multi-site-wifi-infrastructure", "structured-enterprise-network-infrastructure"]);
export const networkingProjects = projects.filter((p) => networkingProjectSlugs.has(p.slug));
